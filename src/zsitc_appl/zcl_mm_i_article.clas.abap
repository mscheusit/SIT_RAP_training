CLASS zcl_mm_i_article DEFINITION
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS:
      ...
      check_article_no IMPORTING is_article TYPE ZSITC_I_MM_Article RAISING zcx_ca_exception_symsg.
...
CLASS ZCL_MM_I_ARTICLE IMPLEMENTATION.
  ...
  METHOD check_article_no.
    ...
    IF sy-subrc = 0.
      MESSAGE e001(zsitc_appl_msg) WITH is_article-ArticleNo INTO DATA(lv_message).
      RAISE EXCEPTION TYPE zcx_ca_exception_symsg
        EXPORTING
          msgid = sy-msgid
          msgno = sy-msgno
          msgty = sy-msgty
